...
class Node:
    ...
    def __init__(self, x=0, y=0, dtype='float', eccentricity=None, sum_total_gravity=0, connected_edges=0):
        """
        Create a new node at x, y.
        dtype: can be one of 'decimal', 'float', 'integer'
        density: should not be set manually
        """
        self.x = x
        self.y = y
        self.dtype = dtype
        self.eccentricity = eccentricity
        self.sum_total_gravity = sum_total_gravity
    ...
    def set_xy_as_dtype(self):
        if self.dtype == 'decimal':
            self.x = Dec(str(self.x))
            self.y = Dec(str(self.y))
    ...
    def set_random_xy(self, lower, upper):
        if self.dtype == 'decimal':
            self.x = Dec(str(numpy.random.randint(lower, upper)))
            self.y = Dec(str(numpy.random.randint(lower, upper)))
        elif self.dtype == 'integer':
            self.x = numpy.random.randint(lower, upper)
            self.y = numpy.random.randint(lower, upper)

    def dist_from_other(self, other):
        return math.hypot((self.x - other.x), (self.y - other.y))

    def scatter(self, colour='red', size=8):
        ...
        if Node.test is True:
            ...
            plt.scatter(self.x, self.y, color=colour, s=size)


class Edge:
    """Represents the edges as having a length and being bounded by two points.
    Edges are deemed to have 'bad' fitness unless assigned otherwise."""
    test = False

    def __init__(self, node_one, node_two, length=None, directed=False, angle=0, fitness='bad', weight=0,
                 node_index=None, total_gravity=0, summed_rank=0, connected=False, minima=0):
        ...
        self.weight = weight
        self.node_index = node_index  # returns the index of this edge based on all edges from node_one
        self.total_gravity = total_gravity
        self.summed_rank = summed_rank
        self.connected = connected
        self.minima = minima

    def __str__(self):
        return f"({self.node_one}, {self.node_two}, Length:{self.length}, Directed:{self.directed}, " \
               f"Angle:{self.angle}, Fitness:{self.fitness}, Weight:{self.weight})"
...
class Path(Sequence):
    """Represents a path as being a series of nodes.
    ...
    def __eq__(self, other):
        ...
        This has been rewritten to instead first check if the sets of the compared paths are equivalent, and break if
        not. Then the second path is rotated so that the paths start on the same node. Then they are checked for
        equivalency.

        if set(path1) == set(path2):
            Start at path1's value in path2, copy to end of path, then go from start of path2 until node before
            original start position.
            compared_path = path2[path2.index(path1[0]):] + path2[0:path2.index(path1[0])]
            if path1 == compared_path:
                print('Path''s are rotationally equivalent.')
        else:
            print('Path''s are not rotationally equivalent.')
        """
        if set(self) == set(other):
            """Start at self's value in other, copy to end of path, then go from start of other until node before 
            original start position."""
            compared_path = other[other.index(self[0]):] + other[0:other.index(self[0])]
            if self == compared_path:
                print('Paths are rotationally equivalent.')
                return True
            else:
                print('Paths are not rotationally equivalent.')
        else:
            print('Paths do not have the same set of elements.')
            return False
    ...
    def average_node(self):
        ...
        # 'Set' is used to remove any duplicate nodes, i.e. the last node usually.
    ...
    #     plt.show()
    def plot(self):
        plt.plot([i.x for i in self.path], [i.y for i in self.path], zorder=2, linewidth=2,
                 color='red')
...
class Instance:
    ...
    def seeder(manual_seed=False, num=None):
        ...
        Below is an example of its usage. It should be set before populating the 'nodes' dictionary. A call to
    ...
    def edge_length(node_one, node_two):
        ...
        return Dec(math.hypot((node_one.x - node_two.x), (node_one.y - node_two.y)))
    ...
    def nodes_by_coord_value(self, reverse=False, coord=None):
        ...
        Default is the lowest values first, set reverse=True for longest first."""
        if coord == 'x':
    ...
    def populate_nodes(self, n):
        ...
        if self.nodes is None:
            self.nodes = {}
            for i in range(n):
                self.nodes[i + 1] = Generator.random_node()
        else:
    ...
    def populate_edges(self, relative_edges=False):
        ...
        if relative_edges is False:
            ...
            if isinstance(self.nodes, list):
                unique_pairs = itertools.combinations(self.nodes, 2)
                for j in range(Instance.pairs_range(len(self.nodes))):
                    index = j
                    pair = next(unique_pairs)
                    self.edges[index + 1] = Edge(pair[0], pair[1])
    ...
    def solve(self, convex_hull=True, brute_force=True):
        ...
        if brute_force is True:
            bf_result = Solvers.brute_force(self)
    ...
    def convex_hull(self):
        ...
        ch_result = Solvers.grahams_scan_convex_hull(self)
        if self.results is None:
    ...
    @property
    def solution(self):
        """Convenience method to return brute-force solution path."""
        return self.results['optimal_solution']


class Generator:

    @staticmethod
    def random_node(dtype='decimal', lower=0, upper=100):
        if dtype == 'decimal':
            return Node(Dec(str(numpy.random.randint(lower, upper))),
                        Dec(str(numpy.random.randint(lower, upper))))
        elif dtype == 'float':
            return Node(float(numpy.random.randint(lower, upper)),
                        float(numpy.random.randint(lower, upper)))
        elif dtype == 'integer':
            return Node(numpy.random.randint(lower, upper),
                        numpy.random.randint(lower, upper))

    @staticmethod
    def random_edge(dtype='decimal', lower=0, upper=1000):
        """
        The 'lower' and 'upper' arguments determine the bounds for the x and y coordinates.
        """
        if dtype == 'decimal':
            return Edge(Node(Dec(str(numpy.random.randint(lower, upper))),
                             Dec(str(numpy.random.randint(lower, upper)))),
                        Node(Dec(str(numpy.random.randint(lower, upper))),
                             Dec(str(numpy.random.randint(lower, upper))))
                        )
        elif dtype == 'float':
            return Edge(Node(float(numpy.random.randint(lower, upper)),
                             float(numpy.random.randint(lower, upper))),
                        Node(float(numpy.random.randint(lower, upper)),
                             float(numpy.random.randint(lower, upper)))
                        )
        elif dtype == 'integer':
            return Edge(Node(numpy.random.randint(lower, upper),
                             numpy.random.randint(lower, upper)),
                        Node(numpy.random.randint(lower, upper),
                             numpy.random.randint(lower, upper))
                        )

    @staticmethod
    def new_instance(node_count=None, empty=False, source='random', file=None, seed='random', solve=False,
                     relative_edges=True, calc_node_densities=False, calc_dmatrix=False, node_dtype=None):
...
class Solvers:
    ...
    def grahams_scan_convex_hull(instance):
        ...
        """
        Returns points on convex hull in counter-clockwise order according to Graham's scan algorithm.
    ...
    @staticmethod
    def find_hamiltonians(edges, method='brute_force'):
        """Returns any Hamiltonian circuits that are present in a provided set of edges.
        Designed to be extended with further methods later if desired.
        """
        if method == 'brute_force':
            nodes = []
            for edge in edges:
                nodes.append(edge.node_one)
                nodes.append(edge.node_two)
            nodes = set(nodes)

            nodes = [[edge.node_one, edge.node_two] for edge in edges.values()]
            pass
        pass
...
class Utilities:
    ...
    def view(data, nodes=False, edges=False, good_edges_only=False, show_avg=False, plot_se=False, plot_le=False,
        ...
        if show_avg:  # plot the average node
            plt.scatter(data.average_node.x, data.average_node.y, marker='.', )

        if plot_se:  # plot the shortest edge
        ...
        if plot_le:  # plot the longest edge
        ...
        # if labels:
        #     for i, j in zip(data.x_values, data.y_values):
